...
function SchoolLogo({ school }: { school: School }) {
  const [logo, setLogo] = useState<{ schoolId: number; url: string }>();
  useEffect(() => {
    let active = true;
    let objectUrl: string | undefined;
    if (!school.logoUrl) return;
    loadSchoolLogoUrl(school.id)
      .then((loaded) => {
        objectUrl = loaded;
        if (active) {
          setLogo({ schoolId: school.id, url: loaded });
        } else {
  ...
  const url =
    school.logoUrl && logo?.schoolId === school.id ? logo.url : undefined;
  ...
  return url ? (
    ...
    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-xs font-bold text-slate-500">
      {initials}
    </span>
  );
}
...
export default function SchoolTable({
  ...
  return (
    <div className="min-w-0 max-w-full rounded-3xl border border-slate-200 bg-white">
