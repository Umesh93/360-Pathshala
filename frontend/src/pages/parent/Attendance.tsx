...
export default function ParentAttendance() {
  ...
  const applyChildren = (records: Child[]) => {
    setChildren(records);
    setSelected((current) =>
      records.some((child) => child.id === current)
        ? current
        : (records[0]?.id ?? 0),
    );
  };
      ...
      .catch((requestError) => {
        ...
        setError(
          message.toLowerCase().includes("forbidden")
            ? "Your account is not linked to a student, or you do not have permission to view child attendance."
            : message,
        );
      })
      .finally(() => {
  ...
  return (
    <ParentLayout>
      <PageHeader
        ...
        subtitle="Attendance for students linked to your guardian account."
        action={
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white">
          <ErrorState
            title="Child attendance unavailable"
            description={error}
            onRetry={reload}
          />
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {children.map((item) => (
              <button
                ...
                className={`rounded-lg border px-4 py-2 text-sm font-medium ${item.id === selected ? "border-[#234A91] bg-[#234A91] text-white" : "border-gray-300 bg-white text-gray-700"}`}
              >
            ...
            <>
              ...
              <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <div className="border-b border-gray-200 p-4">
                  <h2 className="font-semibold text-gray-800">Daily history</h2>
                </div>
                {records.length ? (
