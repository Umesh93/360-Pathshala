...
const fieldClass = (error?: string, readOnly = false) =>
  `h-10 w-full rounded-xl border px-3 text-sm outline-none ${readOnly ? "border-gray-200 bg-gray-50 text-gray-600" : "focus:border-[#234A91] focus:ring-2 focus:ring-blue-100"} ${error ? "border-red-500" : "border-gray-200"}`;
const ErrorText = ({ error }: { error?: string }) =>
  error ? <p className="mt-1 text-xs text-red-600">{error}</p> : null;

const fullName = (...names: (string | undefined)[]) =>
  names
    .map((name) => name?.trim())
    .filter(Boolean)
    .join(" ");
...
export default function GuardianSection({
  ...
  const update = <K extends keyof GuardianData>(
    field: K,
    value: GuardianData[K],
  ) => {
    const next = { ...data, [field]: value };
    if (
      data.guardianSelection === "father" &&
      [
        "fatherFirstName",
        "fatherMiddleName",
        "fatherLastName",
      ...
      next.guardianName = fullName(
        next.fatherFirstName,
        next.fatherMiddleName,
        next.fatherLastName,
      );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <button
              ...
              className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-2 border-dashed border-gray-300 text-xs text-gray-500"
            >
              {data.fatherPhoto ? (
                <img
                  src={String(data.fatherPhoto)}
                  alt="Father"
            ...
            <input
              ...
              accept="image/*"
              onChange={(event) =>
                readPhoto("fatherPhoto", event.target.files?.[0])
              }
              className="hidden"
            />
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <input
              ...
              accept="image/*"
              onChange={(event) =>
                readPhoto("motherPhoto", event.target.files?.[0])
              }
              className="hidden"
            />
          </div>
      ...
      <section className="bg-white rounded-xl p-6 shadow-sm">
        ...
        <div
          data-field="guardianSelection"
          className={`flex flex-wrap gap-6 rounded-xl border p-4 ${errors.guardianSelection ? "border-red-500" : "border-gray-200"}`}
        >
          {(["father", "mother", "other"] as const).map((selection) => (
            <label
              key={selection}
            ...
            >
              <input
                ...
                value={selection}
                checked={data.guardianSelection === selection}
                onChange={() => selectGuardian(selection)}
                className="h-4 w-4 text-[#234A91]"
              />
        ...
        {data.guardianSelection && (
          <div className="mt-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              {data.guardianSelection === "other" &&
                textField("guardianAddress", "Address")}
              {data.guardianSelection === "other" &&
                textField("guardianOccupation", "Occupation")}
              {data.guardianSelection === "other" &&
                textField("guardianCitizenship", "Citizenship")}
            </div>
          </div>
        )}
      </section>
