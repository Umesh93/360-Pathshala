...
export default function SubjectListPage({
  ...
  const [sort, setSort] = useState("subjectName,asc");
  useEffect(() => {
    Promise.all([getAcademicClasses(0, 100), getTeachers(1, 100)]).then(
      ([classPage, teacherPage]) => {
        setClasses(classPage.content);
        setTeachers(
          teacherPage.data.map((item) => ({
            id: item.id,
  ...
  const restore = async (id: number) => {
    try {
      ...
      showToast("Subject restored successfully", "success");
      await load();
    } catch (error) {
      showToast(errorMessage(error), "error");
    }
  ...
  const exportCsv = async () => {
    const blob = await exportSubjectsCsv(filters);
    ...
    <div className="space-y-5">
      {!embedded && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-[#234A91] text-white rounded-xl"
          >
            Add Subject
          </button>
        </div>
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm space-y-3">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          />
          <button onClick={exportCsv} className="px-4 py-2 border rounded-xl">
            Export CSV
          </button>
          <button
            onClick={() => load()}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
          ...
          <select
            ...
            className={filterClass}
          >
            <option value="">Optional / Compulsory</option>
          ...
          <label className={`${filterClass} flex items-center gap-2`}>
            <input
              type="checkbox"
              checked={deleted}
              onChange={(event) => {
            ...
            {loading ? (
              ...
              items.map((item, index) => (
                <tr key={item.id} className="border-t">
                  ...
                  <td className="p-3 font-medium">{item.subjectCode}</td>
                  <td className="p-3">
                    <button
                  ...
                  <td className="p-3">{item.assignedTeacherCount}</td>
                  <td className="p-3">
                    {item.subjectType.replace("_", " + ")}
                  </td>
                  <td className="p-3">{item.fullMarks}</td>
                  <td className="p-3">{item.passMarks}</td>
                  <td className="p-3">{item.creditHours}</td>
                  <td className="p-3">
                    {item.optional ? "Optional" : "Compulsory"}
                  </td>
                  ...
                  <td className="p-3 text-right space-x-3">
                    {item.deleted ? (
                      ...
                      <>
                        <button
                          onClick={() =>
                            navigate(`/admin/subjects/${item.id}/edit`)
                          }
                          className="text-[#234A91]"
                        >
                          Edit
      ...
      <ConfirmDialog
        ...
        description="The subject and its active teacher assignments will be soft deleted."
        confirmLabel="Delete"
        variant="destructive"
        onConfirm={remove}
