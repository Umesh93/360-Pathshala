...
import {
  ...
  encodeGuardianAddress,
  encodeGuardianMetadata,
  getGuardian,
  updateGuardian,
...
export default function GuardianFormPage() {
      ...
      .then((guardian) =>
        setForm({
          ...
          guardianCode:
            ...
            `GDN-${String(guardian.id).padStart(5, "0")}`,
          firstName: guardian.firstName || "",
          middleName: guardian.middleName || "",
          lastName: guardian.lastName || "",
          mobile: guardian.mobile || guardian.phone || "",
          address: decodeGuardianAddress(guardian.address),
        }),
      )
  ...
  const validate = () => {
    ...
    if (!form.address.currentProvince)
      next.currentProvince = "Province is required";
    if (!form.address.currentDistrict)
      next.currentDistrict = "District is required";
    if (!form.address.currentMunicipality.trim())
      next.currentMunicipality = "Municipality is required";
    const ward = Number(form.address.currentWard);
    if (!Number.isInteger(ward) || ward < 1 || ward > 35)
      next.currentWard = "Enter a ward from 1 to 35";
    setErrors(next);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const payload: GuardianRequest = {
        ...
        phone: form.mobile,
        mobile: form.mobile,
        alternativeMobile: form.alternatePhone,
        organization: form.employer,
        address: encodeGuardianAddress(address),
        documents: "",
        documentMetadata: encodeGuardianMetadata(metadata),
      };
      if (id) await updateGuardian(Number(id), payload);
      else await createGuardian(payload);
      showToast(
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <section className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <Field label="Guardian ID" required>
                <input
                  readOnly
                  value={id ? form.guardianCode : "Auto-generated on save"}
              ...
              {textField("mobile", "Mobile", true, "tel")}
              {textField("alternatePhone", "Alternate Phone", false, "tel")}
              {textField("email", "Email", true, "email")}
              {textField("dateOfBirth", "Date of Birth", false, "date")}
              {textField("nationality", "Nationality")}
              {textField("citizenshipNumber", "Citizenship Number")}
              <Field label="Photo">
                <input
                  type="file"
                  accept="image/*"
          ...
          <section className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {textField("employer", "Employer")}
              {textField("annualIncome", "Annual Income")}
              <Field label="Communication Preference">
                ...
                >
                  ...
                  {["SMS", "EMAIL", "PHONE", "WHATSAPP"].map((value) => (
                    <option key={value}>{value}</option>
                  ))}
                </select>
              </Field>
              {textField("preferredLanguage", "Preferred Language")}
            </div>
          </section>
          <AddressForm
            data={form.address}
          ...
          <section className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {textField("emergencyContactPerson", "Contact Person")}
              {textField(
                "emergencyContactNumber",
                "Contact Number",
                false,
              ...
              {textField("emergencyContactRelationship", "Relationship")}
            </div>
          </section>
          <section className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {textField("documentNumber", "Document Number")}
              {textField("documentIssuedDate", "Issued Date", false, "date")}
              {textField("documentExpiryDate", "Expiry Date", false, "date")}
            </div>
            <div className="mt-4">
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-xl border border-gray-200 p-3 text-sm outline-none focus:border-[#234A91]"
              />
            </div>
          </section>
          <div className="flex gap-3">
            <button
              disabled={submitting}
              type="submit"
              className="px-5 py-2 bg-[#234A91] text-white rounded-xl disabled:opacity-60"
            >
              {submitting
                ? "Saving..."
                : id
            ...
            <button
              ...
              className="px-5 py-2 border border-gray-200 rounded-xl"
            >
              Cancel
            </button>
          </div>
