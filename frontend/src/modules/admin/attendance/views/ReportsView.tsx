...
export default function ReportsView({ lookups }: { lookups: Lookups }) {
  ...
  const [filters, setFilters] = useState<
    AttendanceFilters & { reportType: string }
  >({
  ...
  const download = async (value: "csv" | "pdf") => {
    setFormat(value);
    try {
      const blob = await exportReport(value, filters);
      saveBlob(
        blob,
        `attendance-${filters.reportType}-${filters.startDate}-${filters.endDate}.${value}`,
      );
      showToast(`${value.toUpperCase()} report downloaded`, "success");
    } catch (error) {
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_340px]">
      <section className={`${cardClass} p-5`}>
        ...
        <p className="mb-5 text-sm text-gray-500">
          Choose the scope and export a server-generated report.
        </p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <FilterField label="Report type">
            ...
            >
              ...
              <option value="student-detailed">Student detailed</option>
              <option value="teacher-summary">Teacher summary</option>
      ...
      <aside className={`${cardClass} p-5`}>
        <h2 className="font-semibold">Report Contents</h2>
        <ul className="mt-4 space-y-3 text-sm text-gray-600">
          <li>Attendance totals by selected scope</li>
          <li>Present, absent, late, and leave breakdown</li>
          <li>Attendance percentage and working days</li>
          <li>Applied academic, class, and date filters</li>
        </ul>
        <div className="mt-5 rounded-lg bg-blue-50 p-4 text-sm text-blue-800">
          Large reports may take a few moments to prepare. Keep this page open
