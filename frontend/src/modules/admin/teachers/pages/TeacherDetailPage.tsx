...
export default function TeacherDetailPage() {
  ...
  const getFullName = (t: Teacher) =>
    `${t.firstName} ${t.middleName || ""} ${t.lastName}`.trim();
  if (!teacher)
    return (
      <AdminLayout>
        <div className="p-8">Loading teacher...</div>
      </AdminLayout>
    );
  const fields = [
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
          <div>
            ...
            <p className="text-gray-500">{getFullName(teacher)}</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-xl">
            <h2 className="font-semibold">Class & Subject Assignments</h2>
            <p className="text-3xl mt-3">{assignments.length}</p>
          </div>
          <div className="bg-white p-5 rounded-xl">
            <h2 className="font-semibold">Leave Summary</h2>
            <p className="text-3xl mt-3">{leaves.length}</p>
          </div>
          <div className="bg-white p-5 rounded-xl">
        ...
        <div className="bg-white p-5 rounded-xl">
          ...
          <p className="text-sm text-gray-600">
            Teacher record created and last updated through Teacher Management.
          </p>
        </div>
      </div>
    </AdminLayout>
