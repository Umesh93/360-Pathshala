...
const TeacherForm: React.FC<{ teacherId?: number }> = ({ teacherId }) => {
  ...
  if (import.meta.env.DEV)
    console.debug("[TeacherWizard] render", { currentStep });
  ...
  useEffect(() => {
    ...
    Promise.all([generateTeacherId(), generateEmployeeCode()])
      .then(([generatedTeacherId, employeeCode]) =>
        updateSection("employment", {
          ...formData.employment,
  ...
  useEffect(() => {
    if (import.meta.env.DEV)
      console.debug("[TeacherWizard] draft effect", { teacherId });
  ...
  useEffect(() => {
    if (import.meta.env.DEV)
      console.debug("[TeacherWizard] teacher effect", { teacherId });
    if (!teacherId) return;
    ...
    getTeacherById(teacherId)
      .then((teacher) => {
        ...
        loadTeacher({
          ...
          emergency: {
            relationship:
              teacher.relationship || teacher.emergencyRelationship || "",
          ...
          address: {
            ...
            currentMunicipality: teacher.currentMunicipality || "",
            currentWard: teacher.currentWard || "",
            currentStreet: teacher.currentStreet || "",
            permanentSameAsCurrent: teacher.permanentSameAsCurrent || false,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFieldErrors({});
      if (teacherId) await updateTeacher(teacherId, parsed.data);
      else await createTeacher(parsed.data);
      showToast(
        `Teacher ${teacherId ? "updated" : "saved"} successfully!`,
        "success",
      );
      clearDraft();
  ...
  const handleNext = () => {
    if (import.meta.env.DEV)
      console.debug("[TeacherWizard] next click", { currentStep });
    if (import.meta.env.DEV)
      console.debug("[TeacherWizard] validation start", { currentStep });
    // Step transitions currently have no validation gate.
    if (import.meta.env.DEV)
      console.debug("[TeacherWizard] validation end", {
        currentStep,
        valid: true,
      });
    nextStep();
  ...
  const handlePrevious = () => {
    if (import.meta.env.DEV)
      console.debug("[TeacherWizard] previous click", { currentStep });
    prevStep();
  };
